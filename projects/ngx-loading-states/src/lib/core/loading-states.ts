...
export function startLoadingSync(
  loadingStateSubject: LoadingStatesBSubject
): void {
  loadingStateSubject.next({
    error: null,
    loadingState: loadingStates.loading,
...
export function updateLoading<T>(
  loadingStateSubject: LoadingStatesBSubject,
  config?: UpdateLoadingConfig
): pipeFn<T> {
  const { emptyChecker, errorMapper, swallowError, updateOnErrorOnly } =
    config || {};
  return pipe(
    tap((result) => {
